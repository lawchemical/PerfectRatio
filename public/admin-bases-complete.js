...
(function() {
            ...
            <div id="baseModal" class="modal">
                <div class="modal-content" style="max-width: 800px; max-height: 90vh; overflow-y: auto;">
                    ...
                    <form id="baseForm">
                        ...
                        <button type="submit" class="btn btn-primary" style="width: 100%; margin-top: 20px;">Save Base Product</button>
                    </form>
                </div>
            </div>
        `;
    ...
    function renderBases(basesToRender = null) {
        ...
        tbody.innerHTML = bases.map(base => {
            ...
            const ifraDisplay = base.is_dual_purpose && base.ifra_category_2 ? 
                ...
                (base.ifra_category || '-');
            const rating = base.performance_rating ? base.performance_rating.toFixed(1) : '-';
    ...
    async function handleBaseSubmit(e) {
        ...
        data.is_dual_purpose = formData.get('is_dual_purpose') ? true : false;
        ...
        if (data.ease_of_use_rating) data.ease_of_use_rating = parseFloat(data.ease_of_use_rating);
        if (data.performance_rating) data.performance_rating = parseFloat(data.performance_rating);
        ...
        try {
            ...
            const url = isEdit ? 
                `${AdminCore.API_URL}/bases/${data.id}` : 
                `${AdminCore.API_URL}/bases`;
            ...
            if (response.ok) {
                AdminCore.showToast(`Base product ${isEdit ? 'updated' : 'created'} successfully`, 'success');
